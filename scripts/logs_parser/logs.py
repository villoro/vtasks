...
def to_prefect_data(df_in):
    ...
    df["created"] = df["start"].dt.tz_localize(timezone.utc).dtype
    df["exported_at"] = datetime.now(timezone.utc)
    df["start_time"] = df["start"].dt.tz_localize(timezone.utc).dtype
    df["end_time"] = df["end"].dt.tz_localize(timezone.utc).dtype
